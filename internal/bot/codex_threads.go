...
func (a *App) runThreadInputTurnInteractive(ctx context.Context, threadID string, input []codex.InputItem) (string, error) {
	lock := a.threadLock(threadID)
	a.logger.Debug("thread lock wait", "thread_id", threadID, "mode", "input-interactive", "input_preview", previewJSON(input, 800))
	lock.Lock()
	defer lock.Unlock()
	a.logger.Debug("thread lock acquired", "thread_id", threadID, "mode", "input-interactive")
	return a.codex.RunInputTurnWithOptions(ctx, threadID, input, codex.TurnOptions{Effort: "low"})
}
