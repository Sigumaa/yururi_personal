...
func buildCapabilitiesContext(tools []codex.ToolSpec) string {
	...
	lines = append(lines, "- カテゴリ作成、テキストチャンネル作成、rename、topic 更新、チャンネル移動、一括スペース整備、archive 寄せ、チャンネル検索、カテゴリ構造の俯瞰、orphan channel の検出、profile 候補の提案ができる。")
	lines = append(lines, "- SQLite にメッセージ、fact、channel profile、presence、summary、job を保存できる。")
	lines = append(lines, "- open loop、pending promise、routine、curiosity、agent goal、soft reminder、topic thread、initiative、behavior baseline、behavior deviation、反省メモ、成長ログ、判断履歴、自動化候補、context gap、misfire のような長期記憶の下書きを保存し、検索できる。")
	lines = append(lines, "- 定期 job を登録して、release watch、URL watch、daily/weekly/monthly summary、open loop review、curiosity review、initiative review、soft reminder review、topic synthesis review、baseline review、decision review、self improvement review、channel role review、reminder、space review、background Codex task、periodic Codex task のような継続タスクを走らせられる。")
