...
func buildCapabilitiesContext(tools []codex.ToolSpec) string {
	...
	lines = append(lines, "- open loop、pending promise、routine、反省メモ、成長ログ、判断履歴、自動化候補、context gap、misfire のような長期記憶の下書きを保存し、検索できる。")
	lines = append(lines, "- 定期 job を登録して、release watch、URL watch、daily/weekly/monthly summary、open loop review、decision review、self improvement review、channel role review、reminder、space review、background Codex task、periodic Codex task のような継続タスクを走らせられる。")
