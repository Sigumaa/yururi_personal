...
func TestLoadReferenceDocsOrdersAndHashesDeterministically(t *testing.T) {
	dir := t.TempDir()
	...
	bundle1, hash1, err := loadReferenceDocs(dir)
	if err != nil {
		t.Fatalf("load reference docs: %v", err)
	}
	bundle2, hash2, err := loadReferenceDocs(dir)
	if err != nil {
		t.Fatalf("load reference docs second time: %v", err)
	}
